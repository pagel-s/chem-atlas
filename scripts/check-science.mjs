...
assert(scene.includes('const mouthRing =') && scene.includes("labelSprite('the marked channel · ~1 mm across'") && scene.includes("labelSprite('porous washcoat on that wall · ~30 µm thick'") && scene.includes("labelSprite('one Pt nanoparticle · ~3 nm'") && scene.includes("labelSprite('top facet of that same particle · Pt–Pt = 0.28 nm'"), 'Catalyst must mark the selected channel mouth and give every rung of the zoom its real size, so the mm -> nm gap is explicit rather than implied.');
// A hexagon in this lesson means a honeycomb channel. Reusing hexagons as the nm-scale selection
// halo and facet outline made the Pt terrace read as if it sat inside a channel. Selection markers
// at the particle/terrace scale must be circles (many-segment rings), never 6-segment ones.
assert(!scene.includes('particleFacetHalo') && !scene.includes('facetOutline') && scene.includes('new THREE.RingGeometry(.17, .196, 48)') && scene.includes('const particleRing =') && scene.includes('new THREE.RingGeometry(.575, .586, 64)'), 'Catalyst nm-scale selection markers must be circles; hexagons are reserved for real honeycomb channels.');
